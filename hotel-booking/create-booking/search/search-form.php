...
<form method="GET" class="mphb_cb_search_form mphb-search-form" action="<?php echo esc_url($actionUrl); ?>" data-first_available_check_in_date="<?php echo esc_attr($firstAvailableCheckInDate); ?>">
    ...
    <input type="hidden" id="<?php echo esc_attr('mphb_adults-' . $id); ?>" name="mphb_adults" value="<?php echo esc_attr(MPHB()->settings()->main()->getMinAdults()); ?>" />
    <input type="hidden" id="<?php echo esc_attr('mphb_children-' . $id); ?>" name="mphb_children" value="<?php echo esc_attr(MPHB()->settings()->main()->getMinChildren()); ?>" />
